...
import org.axtin.marcely.configmanager2.objects.Comment;
import org.axtin.marcely.configmanager2.objects.Config;
import org.axtin.marcely.configmanager2.objects.Description;
import org.axtin.marcely.configmanager2.objects.EmptyLine;
import org.axtin.marcely.configmanager2.objects.ListItem;
...
public class ConfigPicker {
	...
	public Config addConfig(String path, String value){
		...
		final Config config = new Config(strs[strs.length-1] + ": " + value, strs[strs.length-1], tree, value);
	...
	public Config addConfig(String path, Location value){
		final String[] strs = path.split("\\.");
		
		final Tree tree = path.contains(".") ? getTree(path.substring(0, path.lastIndexOf('.')), true) : file.getRootTree();
		final Config config = new Config(strs[strs.length-1] + ": " +
				value.getX() + "," + value.getY() + "," + value.getZ() + "," + value.getYaw() + "," + value.getPitch() + "," + value.getWorld().getName(),
				strs[strs.length-1], tree, value);
		tree.addChild(config);
		allConfigs.add(config);
	...
	public ListItem addListItem(String path, String value){
		final Tree tree = getTree(path, true);
		final ListItem config = new ListItem(value, tree);
		tree.addChild(config);
	...
	public @Nullable Config getConfig(String path){
		...
		if(tree != null){
			...
			return !file.getConfigNeverNull ? null : new Config(null, null, null, (String) null) /* TODO */;
	}
